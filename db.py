...
def query(query: str, params=()) -> sqlite3.Cursor:
    ...
    if "db" not in g:
        ...
        g.db.row_factory = (
            sqlite3.Row
        )  # Use Row factory to return rows as dict-like objects
...
def verify_user(username: str, password: str) -> bool:
    try:
        ...
        ).fetchone()[0] == hash_password(password):
            session["user"] = (
                username  # Store username in session upon successful login
            )
...
def create_user(username: str, password: str) -> bool:
    ...
    if (
        query("SELECT username FROM users WHERE username = ?", (username,)).fetchone()
        is None
    ):
...
def create_furl(url: str, name: str, user: str) -> str:
    ...
    query(
        "INSERT INTO furled VALUES (?, ?, ?, ?, ?, ?)", (furl, url, name, user, 0, True)
    )
...
def translate_furl(furl: str) -> str | None:
    ...
    res = query(
        "SELECT url FROM furled WHERE furl=? AND active=TRUE", (furl,)
    ).fetchone()
    if res is None:
...
def get_furls(user: str) -> dict[str, tuple[str, str, int, bool]]:
    ...
    return {
        f[0]: f[1:]
        for f in query(
            "SELECT furl, name, url, count, active FROM furled WHERE user=?", (user,)
        ).fetchall()
    }
...
def generate_furl(length=6) -> str:
    while True:
        ...
        if (furl,) not in query("SELECT furl FROM furled").fetchall() and furl not in {
            "signup",
            "home",
            "delete",
            "activate",
